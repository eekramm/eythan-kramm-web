...
allFontOptions.forEach(font => {
  function changeFont() {
    let elementsToChange = document.getElementsByClassName("changeble-text");
    let fontArray = [];
    let fontClassList = font.classList;
    let classListLength = font.classList.length;
    let fontID = font.id;

    if (classListLength == 2) {
      fontArray.push(fontClassList[classListLength - 1]);

      let fontName = fontArray.join();
      let newFontFamily = "" + fontName + ", " + fontID + "";

      for (let element of elementsToChange) {
        element.style.fontFamily = newFontFamily;
      }
    } else if (classListLength == 3) {
      fontArray.push(fontClassList[classListLength - 2]);
      fontArray.push(fontClassList[classListLength - 1]);
    ...
    } else if (classListLength == 4) {
      fontArray.push(fontClassList[classListLength - 3]);
      fontArray.push(fontClassList[classListLength - 2]);
      fontArray.push(fontClassList[classListLength - 1]);
...
document.addEventListener("click", function(event) {
  let isClickInside =
    ...
    fontGallery.contains(event.target) ||
